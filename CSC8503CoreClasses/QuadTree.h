...
namespace NCL {
	...
	namespace CSC8503 {
		...
		protected:
			...
			void StaticDynamicTest(T& object, const Vector3& objectPos, const Vector3& objectSize ,std::vector<CollisionDetection::CollisionInfo> infoList) {
				QuadTreeEntry<T> temp(object, objectPos, objectSize);
				//int count = 0;
				CollisionDetection::CollisionInfo info;
				if (!CollisionDetection::AABBTest(objectPos,
				...
				if (children) { // not a leaf node , just descend the tree
					for (int i = 0; i < 4; ++i) {
						return children[i].StaticDynamicTest(object, objectPos, objectSize, infoList);
					}
				}
				else {
...
namespace NCL {
	...
	namespace CSC8503 {
		...
		public:
			...
			void StaticDynamicTest(T object, const Vector3& pos, const Vector3& size, std::vector<CollisionDetection::CollisionInfo> infoList){
				return root.StaticDynamicTest(object, pos, size,infoList);
			}
