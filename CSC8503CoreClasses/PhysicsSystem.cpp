...
QuadTree <GameObject*> staticTree(Vector2(512, 512), 10, 7);
...
bool PhysicsSystem::broadPhaseHelper(GameObject* a, GameObject* b) {
	...
	if (abs(delta.x) -10 < totalSize.x &&
		//abs(delta.y) < totalSize.y &&
		abs(delta.z) -10 < totalSize.z) {
		return true;
	}
	return false;
...
void PhysicsSystem::createStaticTree() {
	...
	std::cout << "statictree:" << numberStatic;
}
